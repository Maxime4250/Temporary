...
DB* db_init(Person *persons_array, int num_elements) {
    ...
    unsigned long array_start = (unsigned long) malloc((num_elements+1) * sizeof(Person));
    // 2.K : Décaler le début du tableau pour forcer l'alignement
    //array_start = array_start + (unsigned long)abs((int)(32-array_start%64));
	...
	unsigned long address_third = (unsigned long) &(db->persons[2]);
	/*printf("address 1: %lu\n", address_first);
	printf("decalage 1: %lu\n", address_first%64);
	printf("address 2: %lu\n", address_second);
	printf("decalage 2: %lu\n", address_second%64);
	printf("address 3: %lu\n", address_third);
	printf("decalage 3: %lu\n", address_third%64);*/
