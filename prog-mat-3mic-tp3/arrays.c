...
float db_min_dist(DB *db, float lat, float lon) {
	...
	float infinity = 1.0 / 0.0; // infinity
	__m256 min_dist_squared = _mm256_set1_ps(infinity);

    __m256 base_lat = _mm256_set1_ps(lat);
    //print_vec("base lat", base_lat);

	__m256 base_lon = _mm256_set1_ps(lon);
    //print_vec("base lon", base_lon);
	...
	for(int i=0; i<size; i+=8){
		...
		__m256 dxs_squared = _mm256_mul_ps(dxs,dxs);
		__m256 dys_squared = _mm256_mul_ps(dys,dys);
		__m256 dist_squared = _mm256_add_ps(dxs_squared,dys_squared);

		min_dist_squared = _mm256_min_ps(min_dist_squared,dist_squared);
		//print_vec("dist_squared", dist_squared);

	}
	float min_dist = min_float_in_vector(min_dist_squared);
...
void * thread_process_requests_split(void * thread_params) {
	...
	int partageD = params.thread_id*params.db->size/params.num_threads;
    int partageF = (params.thread_id+1)*params.db->size/params.num_threads;
	

    for(int i=0; i<params.num_requests; i++) {
        // 3.I: handle the i-th request if it is on a person we are responsible of
		if(((params.requests->person_id)<partageF)&((params.requests->person_id)>=partageD)){
			db_process_single_request(params.db, &params.requests[i], true);
		}
    }
